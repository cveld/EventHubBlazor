    ...
    public class Sample03_BasicEventProcessing : IEventHubsBlobCheckpointSample
    {
                                   ...
                                   string blobContainerName)
        {
            ...
            try
            {
                ...
                stopWatch.Stop();
                Console.WriteLine($"Processing started. Took {stopWatch.ElapsedMilliseconds} ms");
                ...
                Console.WriteLine($"Cancellation requested: {cancellationSource.IsCancellationRequested}");
                Console.WriteLine($"eventIndex: {eventIndex}");
                ...
                cancellationSource2.CancelAfter(TimeSpan.FromSeconds(10));

                Console.WriteLine("Processing stopping...");
                stopWatch = new Stopwatch();
                stopWatch.Start();
                await processor.StopProcessingAsync(cancellationSource2.Token);
                stopWatch.Stop();
                Console.WriteLine($"Processing stopped. Took {stopWatch.ElapsedMilliseconds} ms. Cancellation: {cancellationSource2.IsCancellationRequested}");
            }
            finally
            {
