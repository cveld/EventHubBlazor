...
namespace Azure.Messaging.EventHubs.Processor.Samples
{
    ...
    {
        private readonly IOptions<EventHubWorkloadConfig> _sampleRunnerConfig;

        public SampleRunner(IOptions<EventHubWorkloadConfig> sampleRunnerConfig)
        {
            _sampleRunnerConfig = sampleRunnerConfig;
        }
        ...
        ///
        public async Task Run(string[] args, int? choice)
        {
            ...
            parsedArgs.EventHubsConnectionString ??= _sampleRunnerConfig.Value.EventHubsNamespaceConnectionString;
            parsedArgs.EventHub ??= _sampleRunnerConfig.Value.EventHub;
            parsedArgs.StorageConnectionString ??= _sampleRunnerConfig.Value.StorageAccountConnectionString;
            parsedArgs.BlobContainer ??= _sampleRunnerConfig.Value.StorageAccountContainer;
            ...
            Console.WriteLine();
            choice ??= ReadSelection(samples.Count);
        ...
        private static void DisplayHelp()
        {
            ...
            Console.WriteLine($"\tAzure.Messaging.EventHubs.Processor.Samples.exe");
            Console.WriteLine();
            ...
            Console.WriteLine();
            Console.WriteLine($"\tAzure.Messaging.EventHubs.Processor.Samples.exe \"{ eventHubsSample}\" \"SomeHub\" \"{ storageSample }\" \"Checkpoints\"");
            Console.WriteLine();
            Console.WriteLine($"\tAzure.Messaging.EventHubs.Processor.Samples.exe { CommandLineArguments.ArgumentPrefix }{ nameof(CommandLineArguments.Help) }");
            Console.WriteLine();
        }
