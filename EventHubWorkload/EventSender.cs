...
namespace EventHubWorkload
{
    ...
    public class EventSender
    {
        ...
        /// </summary>
        ///
        public string Name => nameof(EventSender);
        ...
        public EventSender(IOptions<EventHubWorkloadConfig> sampleRunnerConfig)
        {
            this.eventHubWorkloadConfig = sampleRunnerConfig.Value;
        }

        public async Task SendEvents(string id)
        {
            ...
            {
               new EventData(Encoding.UTF8.GetBytes($"{id}: First Event, First Batch")),
               new EventData(Encoding.UTF8.GetBytes($"{id}: Second Event, First Batch")),
               new EventData(Encoding.UTF8.GetBytes($"{id}: Third Event, First Batch")),

               new EventData(Encoding.UTF8.GetBytes($"{id}: First Event, Second Batch")),
               new EventData(Encoding.UTF8.GetBytes($"{id}: Second Event, Second Batch")),
               new EventData(Encoding.UTF8.GetBytes($"{id}: Third Event, Second Batch")),

               new EventData(Encoding.UTF8.GetBytes($"{id}: First Event, Third Batch")),
               new EventData(Encoding.UTF8.GetBytes($"{id}: Second Event, Third Batch")),
               new EventData(Encoding.UTF8.GetBytes($"{id}: Third Event, Third Batch")),
            };
            ...
            Func<Task> sendEvents = async () =>
            {
                await using (var producer = new EventHubProducerClient(eventHubWorkloadConfig.EventHubsNamespaceConnectionString, eventHubWorkloadConfig.EventHub))
                {
            ...
            await StopWatch(async () => await sendEvents(), "Sending events", "Sent events");
        }
                                   ...
                                   string blobContainerName)
        {
            ...
            try
            {
                ...
                await StopWatch(async() => await processor.StopProcessingAsync(), "Stopping processing", "Stopped processing");                
            }
            finally
            {
        ...
        async Task StopWatch(Func<Task> func, string start, string stop)
        {
            ...
            await func();
            stopwatch.Stop();
            Console.WriteLine($"{stop}. Took {stopwatch.ElapsedMilliseconds} ms.");
        }
    }
}
